...
int main(int argc,char *argv[]){
    ...
    do{
        ...
        while(bytes_sent < FILE_SIZE){
            ssize_t result = send(sock, random_file + bytes_sent, FILE_SIZE - bytes_sent, 0);
            if (result < 0) {
                perror("send(2)");
                close(sock);
                free(random_file);
        ...
        fprintf(stdout, "Sent %ld bytes to the receiver!\n"
    ...
    if (send(sock, EXIT_MESSAGE, strlen(EXIT_MESSAGE)+1, 0) == -1){
        perror("send(2)");
        free(random_file);
        close(sock);
        return 1;
